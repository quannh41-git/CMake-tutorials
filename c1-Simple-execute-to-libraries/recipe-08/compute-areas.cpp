...
int main()
{
    ...
    int nSides = 19;
    double side = 1.29312;
    double A_polygon = area::polygon(nSizes, side);
    std::cout << "A regular polygon of " << nSizes << " sides of length "
              << side << " has an area of " << A_polygon << std::endl;

    double d1 = 5.0;
    double d2 = 7.8912;
    double A_rhombus = area::rhombus(d1, d2);
    std::cout << "A rhombus of major diagonal " << d1 << " and minor diagonal "
              << d2 << " has an area of " << A_rhombus << std::endl;
